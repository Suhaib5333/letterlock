...
| B3 | **Phase 0 paperwork** (in progress) | ✅ Company Apple Account created + verified 2026-09-07 (browser-only, never signed into a phone's iCloud; whichever account enrols permanently owns the listing). ✅ D-U-N-S `561683753` (`RAL SOFTWARE SERVICES`, Bahrain), **usable from 2026-09-21**. 🔄 **2026-09-24: both stores paid and in review.** Apple $99 paid, passport uploaded, ID under review (the region was locked to Australia and Organization was refused, so it went in as Sole Proprietor; the enrolment type and the seller name are still to be confirmed from Apple's approval email). Play $25 paid as Organization, verification docs uploaded, in review (the Android-device check in the Play Console app may still be asked). ⏳ Still open: the Play merchant account, RevenueCat, the Google OAuth consent screen, and reserving the name in App Store Connect after approval. The CR is a **sole proprietorship** (confirmed 2026-09-24), so every US tax form (Apple, Play, AdMob) is the individual **W-8BEN** in Suhaib's own name, not W-8BEN-E. LAUNCH_PLAN §3 Phase 0; every outstanding identifier is in `docs/ACCOUNTS.md`. | Phases 4, 5, 6 going live (the code is already written) |
