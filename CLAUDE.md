...
| B13 | **The iOS build has never been run** | ✅ **Unblocked without the D-U-N-S 2026-09-08:** `codemagic.yaml` gained an **`ios-smoke`** workflow (unsigned, simulator slice, no App Store Connect integration), so the Xcode 26 + Capacitor 8 SPM chain can be proven TODAY on Codemagic's free tier with no Apple membership. Sign up at codemagic.io with GitHub, connect the repo, run `ios-smoke` by hand (or push an `ios-smoke-*` tag). A signed TestFlight build still needs the paid account. `codemagic.yaml` is now covered by the CI YAML parse gate. | First store build (signed only) |
