...
| D2 | **Developer account** | **Organization: RAL SOFTWARE SERVICES** on both stores (**revised 2026-09-14**: the D-U-N-S and the Bahrain CR both read RAL SOFTWARE SERVICES, so that is the legal entity and the public developer name. "RAL Technologies" was never the legal name and cannot be used.) | Company name on the listing; Play organization accounts skip the 12-testers / 14-day closed-test rule. |
...
| D11 | **Fandom packs** | **Keep** as text trivia, "Unofficial fan trivia, not affiliated" label, franchise names never in store metadata. **Revised 2026-09-10 at Suhaib's request:** the Harry Potter pack is called *Harry Potter* in-app again (it was *Wizarding School Trivia*); the Pokemon pack keeps its descriptive title. | Facts are not copyrightable. The label and keeping franchise names out of store metadata are the mitigations that carry the weight; an in-app pack name describing the subject is nominative use. |
...
| Step | Detail |
|---|---|
| Add Capacitor 8 | `npm i @capacitor/core @capacitor/cli`, `npx cap init "Letterlock" dev.raltech.letterlock --web-dir dist`, `npx cap add ios android`. Requirements: Node 22+, Xcode 26 (**Suhaib has a Mac as of 2026-09-14, so local `npx cap open ios` is the primary path**; Codemagic remains the reproducible CI path), Android Studio, minSdk 24, targetSdk 36. `server.hostname: "letterlock.raltech.dev"` so the WebView origin matches our domain. |
...
| CI builds | **Codemagic** (500 free macOS minutes/month; optional now that a Mac is available, but keep it so store builds are reproducible off one laptop) with `codemagic.yaml`: iOS signing via App Store Connect API key → TestFlight; Android AAB → Play internal track. Ionic Appflow is shutting down (do not use). |
