...
| 🎼 **Guess the Melody** (`melodies`) | **Synthesized public-domain tunes** | `scripts/genclips.mjs` renders PD melodies to bundled WAVs in `public/clips/`. Ships everywhere (apps + web). | PD compositions; our rendition. |
| 🎼 **Guess the Melody · Themes** (`melodies-itunes`) | **iTunes Search API** previews | `scripts/genmelodies_itunes.mjs` → `melodiesExtra.ts`; a separate pack since 2026-09-05 so D3 can gate it: **web only** (`platforms: ['web']`, set automatically by `content/index.ts` for any pack carrying iTunes media). | Apple 5.2.5 forbids iTunes previews in a store build. |
| 🏳️ **World Flags** + regional cards + Sports flag questions | **Bundled SVGs** in `public/flags/` | `scripts/genflags.mjs` downloads ~175 country SVGs locally. **flagcdn.com is blocked on some networks** → we bundle; the Sports packs reuse the same `/flags/<iso2>.svg` paths (no flagcdn anywhere since 2026-09-05). Regional pack cards show the bundled flag image (emoji renders as "BH/SA/AE" letters on Windows). | Public-domain flag SVGs. |
| 🔷 **Guess the Logo** (easy/med/hard) | **Simple Icons, bundled** in `public/logos/<slug>.svg` | `scripts/genlogos.mjs` reads every `q('Brand','slug')`, downloads the SVG once from the Simple Icons CDN, drops any slug that fails and the Apple logo (D10), and points the packs at `/logos/<slug>.svg`. 502 SVGs, 1.8 MB. Shown on a light panel so any color shows; the card + group carry "All logos are trademarks of their owners". | Simple Icons are CC0; the marks stay their owners' trademarks (identification use, removal on request). |
| 🎭 **Charades** (5 packs, ~1,107 prompts) | **Bundled licensed photos** in `public/charades/<packId>/<slug>.webp` (512 px, ≤ 40 KB) | `scripts/genimages.mjs` (D9 / Phase 1c) fetches ONE image per prompt: Pixabay when `PIXABAY_KEY` is set, else Wikimedia Commons (Wikidata item → `haswbstatement:P180` "depicts" search, then text search; only PD/CC0/CC BY/CC BY-SA, bitmaps only, safety word filter, no logos/posters), else Openverse (cc0/pdm, rate-capped), else the prompt stays **word-only**. Writes `credits.json` per pack + `src/content/charadesImageManifest.ts` (which slugs have a file) + review contact sheets in `docs/charades-review/`. `reject.txt` per pack forces a re-fetch (`slug`) or word-only (`slug !`). `withCharadeImages()` attaches `/charades/<packId>/<slug>.webp` only when the file exists; the card shows a **QR → `/?view=img`** secret-prompt page (word + image, privately). loremflickr is gone. | Each image is credited (author, license, source) in Settings/Home → "Image credits" (`ImageCredits.tsx`, reads the credits.json files lazily). The WORD is still the content: a missing/failed image hides itself. |
| 🔤 **Fonts** (Bricolage Grotesque, Sora, Tajawal, Atkinson Hyperlegible, Lexend) | **Bundled woff2** in `public/fonts/` via `src/fonts.css` | `scripts/genfonts.mjs` asks the Google Fonts CSS API for our exact request, keeps latin/latin-ext/arabic subsets, downloads 18 files (392 KB) and rewrites the `@font-face` rules to `/fonts/`. No `fonts.googleapis.com` / `gstatic` request at boot (verified by Playwright). | SIL OFL 1.1 (`public/fonts/LICENSES.md`). |
...
(iTunes song previews), `genlogos.mjs` (bundle Simple Icons SVGs to `public/logos/`), `genflags.mjs` (bundle flag
SVGs), `genimages.mjs` (bundle licensed charades images + credits + review sheets), `genfonts.mjs`
(bundle web fonts), `genclips.mjs` (synth PD melody WAVs), `checkmedia.mjs` (the media GATE: every
local media path exists, remote iTunes URLs reachable, charades coverage + ≤ 60 KB, no forbidden
third-party host in `src/` or `index.html`; exits non-zero),
